...
CLAAMatrix=CLAAMatrix(:,x_range);
DeltaCMatrix=DeltaCMatrix(:,x_range);
DeltaAMatrix=DeltaAMatrix(:,x_range);
DeltaMgMatrix=DeltaMgMatrix(:,x_range);
DeltaCO3Matrix=DeltaCO3Matrix(:,x_range);
...
ap = ax(2).Position;
cbar(1).Position = [ap(1)+ap(3)-2e-2,0.3,0.01,0.5];
text(ax(2),-0.15,1.05,panl{1},'FontSize',FontSize_Panel,'Color','k','Units','normalized')

% plot Mg map and Carbonate Map only
for ii = 2:3
    ax(ii) = subplot(2,4,ii+[0,4]);
    ...
    ap = ax(ii).Position;
    cbar(ii).Position = [ap(1)+ap(3)-2e-2,0.3,0.01,0.5];
    text(ax(ii),1.3,0.95,cbar_str{ii},...
    'Units','normalized',...
    'FontSize',FontSize_Tick,...
    'Color','k',...
    'HorizontalAlignment','center');

    text(ax(ii),-0.15,1.05,panl{ii},'FontSize',FontSize_Panel,'Color','k','Units','normalized')
end
...
text(ax(ii),-0.15,1.1,panl{ii},'FontSize',FontSize_Panel,'Color','k','Units','normalized')

% plot group means for predicted carbonate map as a bar plot
ax(ii+1) = subplot(2,4,ii+4);

boxplot(Mp(:,ii+1),cats(Gp),...
        'Notch','on',...
        'DataLim',[-0.6,0.6],...
        'ExtremeMode','clip');hold on; %
...
yline(-std_val(ii+1),'k:','LineWidth',1); 
xlabel('cluster ID','FontSize',FontSize_Label)
text(ax(ii+1),-0.15,1.1,panl{ii+1},'FontSize',FontSize_Panel,'Color','k','Units','normalized')
...
for ii = 1:size(Mp,2)
    [~,~,stats] = anova1(Mp(:,ii),cats(Gp),'off');
