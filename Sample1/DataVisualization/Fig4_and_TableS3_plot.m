...
imagesc(ax(2),yv,xv,rot90(G,3),'alphadata',alpha_val*rot90(tmap,3));
daspect([yAsp,xAsp,1])
colormap(ax(2),cmap);
caxis(ax(2),[0.5,3.5]);
ax(2).Visible = 'off';
...
text(ax(2),0,1.05,panl{1},'FontSize',FontSize_Panel,'Color','k','Units','normalized')

% plot Mg map and Carbonate Map only
for ii = 2:3
    ax(ii) = subplot(2,16,4*(ii-1)+[1:3,17:19]);
    %imagesc(xv,yv,M(:,:,ii+2));
    imagesc(yv,xv,rot90(M(:,:,ii+2),3)); 
    ax(ii).TickDir = 'out';
    daspect([yAsp,xAsp,1])
    ...
    text(ax(ii),1.2,.75,cbar_str{ii},...
    'Units','normalized',...
    'FontSize',FontSize_Tick,...
    'Color','k',...
    'HorizontalAlignment','center');

    text(ax(ii),0,1.05,panl{ii},'FontSize',FontSize_Panel,'Color','k','Units','normalized')
end
